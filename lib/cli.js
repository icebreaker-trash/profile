      ...
      if (value === options.profile) {
        ...
        await new Promise((resolve, reject) => {
          const showPhoto = (idx = 0) => {
            const photo = fs.readFileSync(path.resolve(__dirname, `photo/${idx}.txt`), {
              encoding: 'utf-8'
            })
            log('\n')
            log(photo)
            log(`\n上一张: ${chalk.bold.greenBright('←')} 下一张: ${chalk.bold.greenBright('→')} 退出请按: ${chalk.bold.greenBright('ctrl + c')}`)
          }
          const total = 6
          let idx = 0
          showPhoto(idx)
