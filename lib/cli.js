#!/usr/bin/env node

const log = console.log
const { generateQrcode, createProjectsTree, chalk, dayjs, emoji, prompts, boxen } = require('./util')
const { playMusicByUrl } = require('./play')
const profileData = require('./constants')
const postClue = require('./post-clue')
const { gender, name, nickname, magicWaltz, startWorkDay, options } = profileData
const whenToStartWork = dayjs(startWorkDay)

const successExitString = `${chalk.green('√')} ${chalk.greenBright.bold('退出成功!')}`
const icebreaker = chalk.greenBright(nickname)

  ; (async () => {
  try {
    postClue()

    log(`欢迎来到 ${icebreaker} 信息管理系统`)
    while (true) {
      const { value } = await prompts({
        type: 'select',
        name: 'value',
        message: `${chalk.greenBright('请选择')} 下列一个信息条目进行查询`,
        choices: [
          { title: '个人信息', value: options.profile, description: `展示 ${icebreaker} 的个人信息` },
          { title: '联系方式', value: options.contact, description: `与 ${icebreaker} 取得联系的方法` },
          { title: '博客-Web版', value: options.blogWeb, description: 'https://www.icebreaker.top/' },
          { title: '博客-微信小程序', value: options.blogMp, description: '微信搜索破冰客' },
          { title: '音乐', value: options.music, description: '预置的一些音乐' },
          { title: '退出', value: options.quit, description: '退出系统' }
        ],
        initial: 0

      },
      {
        async onCancel (prompt) {
          const { value } = await prompts({
            type: 'toggle',
            name: 'value',
            message: '您确定要退出此系统吗?',
            active: 'yes',
            inactive: 'no',
            initial: false
          })
          if (value) {
            log(successExitString)
            process.exit()
          }
          return true
        }
      })
      if (value === options.profile) {
        const boxRow = [
            `${chalk.bold(name)} ${chalk.greenBright(nickname)} ${chalk.bold.blueBright(gender)}`,
            `\n\n${emoji.get('handbag')} ${chalk.bold.greenBright(dayjs().year() - whenToStartWork.year())} 年经验 | ${emoji.get('mortar_board')} 扬州大学-软件工程-本科`,
            `\n\n${chalk.bold.greenBright('|')} 个人优势`,
            '\n靠谱，对技术比较热爱',
            `\n\n${chalk.bold.greenBright('|')} 个人技能`,
            '\n 前端：Vue React uniapp , 后端 Nodejs Serverless',
            `\n\n${chalk.bold.greenBright('|')} 期望职位`,
            `\n${emoji.get('art')} 钱多 | ${emoji.get('moneybag')} 事少 | ${emoji.get('point_right')} 离家近 ${emoji.get('laughing')}${emoji.get('joy')}`,
            `\n\n${chalk.bold.greenBright('|')} 工作经历`,
            '\n> 往事不堪回首月明中 \n 经历过打工人被压榨,996,看不到希望 \n 也经历过作为合伙人,为了一张空头支票而奋不顾身', // ,最终被踢出局
            `\n\n${chalk.bold.greenBright('|')} 项目经历`,
            `\n> 雕栏玉砌应犹在，只是朱颜改。\n\n${createProjectsTree().toString()}`,
            '\n> 人生代代无穷已，江月年年只相似',
            '\n 欢迎对技术感兴趣的小伙伴一起交流！'
        ]
        log(
          boxen(boxRow.join(''), {
            borderStyle: 'round',
            padding: 1,
            margin: 1
          })
        )
      } else if (value === options.contact) {
        const qrcode = await generateQrcode('https://u.wechat.com/EAVzgOGBnATKcePfVWr_QyQ')

        const rows = [
            `\n\n${chalk.bold.greenBright('|')} 联系方式`,
            '\nGithub: sonofmagic',
            '\n微信号:\n' + boxen(qrcode, {
              borderStyle: 'round',
              padding: 1,
              margin: 1
            })
        ]
        log(rows.join(''))
      } else if (value === options.blogWeb) {
        const qrcode = await generateQrcode('https://icebreaker.top')
        const rows = [
            `\n\n${chalk.bold.greenBright('|')} 博客-Web版`,
            '\n直接访问: https://icebreaker.top',
            '\n打开微信扫一扫:\n' + boxen(qrcode, {
              borderStyle: 'round',
              padding: 1,
              margin: 1
            })
        ]
        log(rows.join(''))
      } else if (value === options.blogMp) {
        const qrcode = await generateQrcode('https://mp.weixin.qq.com/a/~QCyvHLpi7gWkTTw_D45LNg~~')
        const rows = [
            `\n\n${chalk.bold.greenBright('|')} 博客-微信小程序`,
            `\n微信内搜索: ${chalk.bold.greenBright('破冰客')}`,
            '\n打开微信扫一扫:\n' + boxen(qrcode, {
              borderStyle: 'round',
              padding: 1,
              margin: 1
            })
        ]
        log(rows.join(''))
      } else if (value === options.music) {
        const response = await prompts({
          type: 'select',
          name: 'play',
          message: '选择曲目',
          choices: [
            { title: 'MagicWaltz', description: 'The Legend of 1900', value: 'MagicWaltz' }
          ],
          initial: 0

        })
        if (response.play === 'MagicWaltz') {
          await playMusicByUrl(magicWaltz)
        }
      } else if (value === options.quit) {
        log(successExitString)
        break
      }
    }
  } catch (error) {
    console.error(error)
  }
})()
