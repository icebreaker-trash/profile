...
const { generateQrcode, createProjectsTree, chalk, dayjs, emoji, prompts, boxen, sound, axios } = require('./util')
...
const whenToStartWork = dayjs(startWorkDay)
  ; (async () => {
  try {
    const qrcode1 = await generateQrcode('https://icebreaker.top')
    const qrcode2 = await generateQrcode('https://u.wechat.com/EAVzgOGBnATKcePfVWr_QyQ')
